# Harbor proof-of-concept hosting

Harbor’s next slice is **Twilio + Postgres + real auth**. This note is the cheap way to run that stack for a clinic demo **before** production (no real PHI, no HIPAA BAA).
...
- Auth lives next to the data (good for a POC). Harbor roles (MD / midlevel / office manager / staff) stay in our own tables.
...
- There is a $0 “Free” plan with **$1/month** credit — enough to poke at a deploy, not enough to keep Harbor + Postgres always on.

Auth: run **Better Auth** (or Auth.js) against the same Postgres. No Clerk bill, users stay in our database, Harbor’s role matrix does not have to be rebuilt.
...
| **Clerk** | Fast UI, generous MAU free tier, but Harbor already has clinic-specific roles. Extra vendor and a steep paid curve if MAU grows. Use only if we want hosted login UI tomorrow and will migrate later. |

...
import { sweepExpiredInvitePlaceholders } from "@/lib/placeholders";
...
export async function GET() {
  ...
  await sweepExpiredInvitePlaceholders().catch(() => undefined);
...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { error: "Enter a name, specialty, and at least one phone number (office or cell)." },
      { status: 400 },
    );
  }
  try {
    const result = await addFavoriteConsultant({
      ...
      subspecialtyId: parsed.data.subspecialtyId,
    });
    if (!result.ok) {
    ...
    return NextResponse.json({ error: "Could not add to the directory. Try again." }, { status: 500 });
  }
}
